...
class leaderProfile extends React.Component{
    ...
    async componentWillMount(){
        ...
        if(this.props.leaderProfileLoaded)
    {
        var leadersss =  this.props.leaderProfile.map(d=><Card 
            id={d.id}
             /> )
        this.setState({
            leaders:leadersss
...
const mapStateToProps = (state) => {
  ...
  const mapDispatchToProps = (dispatch) => {
    return{
        leaderProfile : (profile_info) => dispatch(leaderProfileAction.leaderProfile(profile_info))
    }
  }
