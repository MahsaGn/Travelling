...
class userProfile extends React.Component {
  ...
  async componentWillMount() {
    ...
    if (this.props.is_leader) {
      let placess = "";
      console.log("in user profileeeeeeeee", this.props.data.place);
      ...
      this.setState({
        ifIsLeader: [
          <Profile_item
            title="جنیست"
            val={this.props.data.gender ? "آقا" : "خانم"}
          />,
  ...
  render() {
    return (
      <dev id="profilePage">
        ...
        {this.props.data.is_leader ? (
          <Link to="/addPlaceForLeaderForm">
            <Button>اضافه کردن مکان</Button>
          </Link>
        ) : null}
        {this.props.data.travelouges != undefined ? (
          <lable>سفرنامه ها</lable>
        ) : null}
