...
class userProfile extends React.Component {
    ...
    async componentWillMount() {
        ...
        if(this.props.is_leader){
            let calander = <button onClick={()=>window.location.replace('/mycalender')}>مشخص کردن زمان هایی که هستم</button>
            let placess = ""
...
const mapDispatchToProps = (dispatch) => {
    return {
        userProfile: (profile_info) => dispatch(userProfileAction.userProfile(profile_info))
    }
}
